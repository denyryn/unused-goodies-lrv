@php
    $navLinks = [
        [
            ...
            'active' => request()->routeIs('home_page'),
        ],
        [
            'name' => 'Category',
            'href' => route('category_page'),
            'active' => request()->routeIs('category_page'),
        ],
        [
            'name' => 'Product',
            'href' => route('product_page'),
            'active' => request()->routeIs('product_page'),
        ],
    ];
@endphp
<nav x-data="{ open: false }" class="bg-theme border-gray-200 dark:border-gray-950 shadow">
    <div class="navbar text-theme-invert">
        <div class="navbar-start">
            <a href="{{ route('home_page') }}" class="btn btn-ghost w-fit h-fit">
        ...
        <div class="hidden navbar-center sm:flex ">
            <ul class="px-1 font-bold menu menu-horizontal font-rubik space-x-1">
                @foreach ($navLinks as $link)
                    <li><x-nav-link href="{{ $link['href'] }}" :active="$link['active']">{{ $link['name'] }}</x-nav-link></li>
                @endforeach
            </ul>
        </div>
        <div class="navbar-end space-x-1">
            @if (\App\Utilities\Permission::isLoggedIn())
                <livewire:cart.quick-view-cart>
            @endif

            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <!-- Teams Dropdown -->
                @if (\App\Utilities\Permission::isLoggedIn() && Laravel\Jetstream\Jetstream::hasTeamFeatures())
                ...
                @else
                    <a href="{{ route('login') }}" class="btn btn-ghost ">
                        <img class="size-5 dark:invert"
                            src="{{ \App\Utilities\Asset::getFromAssets('Icons/user.svg') }}" alt="User Login">
                    </a>
                @endif
            </div>
            ...
            <div class="-me-2 flex items-center sm:hidden">
                <button @click="open = ! open"
                    class="inline-flex btn btn-ghost items-center justify-center p-2 rounded-md text-gray-800 dark:text-gray-200 hover:text-gray-900 dark:hover:text-gray-100 focus:outline-none focus:text-gray-900 dark:focus:text-gray-100 transition duration-150 ease-in-out">
